...
export default function BurnModal ({ isOpen, setIsOpen, onClose, balance, availableAmount }) {
    const { register, formState: { isDirty, isValid, errors }, handleSubmit, watch } = useForm({ defaultValues, mode: 'onChange', resolver: joiResolver(schemas.burn)});
    ...
    return (
        ...
        <div className="modal-card" style={styles.modalCard}>
            ...
            <p className="modal-card-title" style={styles.modalCardTitle}>Burn NFT</p>
            </header>
            <section className="modal-card-body">
                ...
                {
                    watchAmount > balance ? (
                        <p>Error: Cannot burn more tokens than you own ({balance})</p>
                    ) : <></>
                }
            </section>
            <footer className="modal-card-foot">
            <button className="button" disabled={(!isValid && isDirty) || watchAmount > balance} onClick={handleSubmit(closeModal)}>Burn</button>
            </footer>
        </div>
        </div>
    )
