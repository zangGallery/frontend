...
export default function TransactionNotifications () {
    ...
    const createNotification = (transactionId) => {
        toast.loading(<></>, {
            ...
            pauseOnHover: false,
            progress: undefined,
            closeButton: <p><span onClick={() => toast.dismiss(transactionId)}><FontAwesomeIcon icon={faTimes} /></span></p>,
    ...
    const onTransaction = (transactionId, status) => {
        const defaultTitle = <h1 className="is-size-5 has-text-black">{status.name}</h1>;
        const viewOnExplorer = status.hash ? <p><ViewOnExplorer hash={status.hash} /></p> : <></>;
